        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {serverHealth.message || 'Verificando conectividad del sistema...'}
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border dark:border-gray-700">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {healthData?.runtime ? `${healthData.runtime} v${healthData.version}` : 'Servicio de scraping'}
            </p>
            {serverHealth.details.healthCheck?.error && (
              <p className="text-xs text-red-500 dark:text-red-400 mt-1">
                Error: {serverHealth.details.healthCheck.error}
              </p>
            )}
          </div>
          <div className="flex items-center">
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border dark:border-gray-700">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {testData?.server || 'Endpoints de scraping'}
            </p>
            {serverHealth.details.apiTest?.error && (
              <p className="text-xs text-red-500 dark:text-red-400 mt-1">
                Error: {serverHealth.details.apiTest.error}
              </p>
            )}
          </div>
          <div className="flex items-center">
      ...
      {serverHealth.status === 'degraded' && (
        <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
          <p className="text-sm text-yellow-800 dark:text-yellow-200">
            ⚠️ Sistema parcialmente operativo. El scraping puede funcionar con limitaciones.
          </p>
        </div>
      )}
      ...
      {serverHealth.status === 'down' && (
        <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
          <p className="text-sm text-red-800 dark:text-red-200">
            ❌ Backend no disponible. Verifica que el servidor esté ejecutándose correctamente.
          </p>
          <p className="text-xs text-red-700 dark:text-red-300 mt-1">
            🔗 Configuración HTTP: Frontend puerto 3080, Backend puerto 3081
          </p>
        </div>
      )}
    </div>
